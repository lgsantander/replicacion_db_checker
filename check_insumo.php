<?php
		...
		while ($row1 = $result1->fetch_assoc()) {
			...
			if ($row1 !== $row2) {
				...
				foreach($row1 as $key => $value){
					writeLog("SERVER 4 => $key: $value <-- AND --> SERVER7 => $key:{$row2[$key]}");
				}

				// writeLog(json_encode(['SERVER 4'=>$row1]) . "\n\n");
				// writeLog(json_encode(['SERVER 7'=>$row2]) . "\n");
			}
		}
	}
	...
	compareTables($conn1, $conn2, 'veisolicitudes');

	$conn1->close();
	$conn2->close();


	function writeLog($message, $logFile = './check_insumo.log') {
		print $message;
